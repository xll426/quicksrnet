...
opt = {
    'scale': 2,  # 超分辨率缩放因子
    'model_checkpoint': 'D:\\SR\\quicksr\\x8\\checkpoints_small\\checkpoint_float32.pth.tar',  # 模型检查点路径
    'output_video': './output_SR_ori_video.mp4',  # 输出超分辨率视频路径
    'input_video': './case20_xgm_outdoor_night_car_1920x1088_25fps_nv12.h265',  # 输入H265视频路径
}
...
def prepare_output_video(input_video_path, output_video_path, fps):
    ...
    frame_width = int(input_stream.get(cv2.CAP_PROP_FRAME_WIDTH)) * opt['scale']
    frame_height = int(input_stream.get(cv2.CAP_PROP_FRAME_HEIGHT)) * opt['scale']
